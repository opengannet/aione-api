...
func TestAdvancedCustomOpenAICompletionsEndpointType(t *testing.T) {
	config := &AdvancedCustomConfig{
		Routes: []AdvancedCustomRoute{
			{
				IncomingPath: "/v1/completions",
				UpstreamPath: "/v1/completions",
				Models:       []string{"flyte-managed-model"},
			},
		},
	}
	require.NoError(t, config.Validate())
	...
	assert.Equal(t, []types.EndpointType{
		types.EndpointTypeOpenAI,
	}, config.SupportedEndpointTypesForModel("flyte-managed-model"))
	assert.Empty(t, config.SupportedEndpointTypesForModel("other-model"))
}
